...
export class TechInfo extends React.Component <{ match: any, data: PackLoadedData }, TechInfoState> {
    ...
    public componentDidUpdate(prevProps: { match: any, data: PackLoadedData }, prevState: TechInfoState) {
    ...
    public shouldComponentUpdate(nextProps: Readonly<{ match: any; data: PackLoadedData }>, nextState: Readonly<TechInfoState>, nextContext: any): boolean {
        console.log("shouldComponentUpdate(", nextProps, nextState, ")");
        return this.props.match.params.techName !== nextProps.match.params.techName
    ...
    public render() {
        ...
        const {tech, otherEffects, techRequired, techAllows, recipeUnlockedBy} = this.state;
        ...
        return <Container>
            <div>
                {tech !== null &&
                <h2><PrototypeIcon item={tech}/> {tech.title}</h2>
                ||
                <h2>{techName}</h2>
                }
                ...
                {techRequired.length > 0 && (
                    <div>
                        ...
                        {techRequired.map(obj =>
                            <div key={obj.name}>
                                <PrototypeLink item={obj} to={data.link.toTech(obj)} data={data}/>
                            </div>
                        )}
                        <br/>
                    </div>
                ...
                {recipeUnlockedBy.length > 0 && (
                    <div>
                        <h3>Unlocks</h3>
                        <RecipeList data={data} recipes={recipeUnlockedBy} noTechUnlocks/>
                    </div>
                )}
            </div>
        </Container>
